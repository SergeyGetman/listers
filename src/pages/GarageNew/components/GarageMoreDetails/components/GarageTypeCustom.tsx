...
import { FormCustomBlock } from '../../types';
...
interface IGarageTypeCustomProps {
  readySubmit?: boolean;
}

const GarageTypeCustom: FC<IGarageTypeCustomProps> = ({ readySubmit }) => {
  const dispatch = useAppDispatch();

  const { control, setValue, getValues } = useForm<FormCustomBlock>({
    defaultValues: rest.initialValueCustomBlock,
  ...
  useEffect(() => {
    setValue('type_of_vehicle', engine?.model);
              ...
              <Grid xs={12} sm={6} item>
                <Controller
                  ...
                  render={({ field, fieldState }) => (
                    <MuiBaseTextFiled
                      label={t('garageHub.moreDetailsBlock.moreDetailsSelectsLabel.typeOfVehicle')}
                      placeholder={t('garageHub.moreDetailsBlock.moreDetailsSelectsLabel.typeOfVehicle')}
                      isSearchable
                      isClearable
                      options={rest.vehicleOptions}
...
export default GarageTypeCustom;
