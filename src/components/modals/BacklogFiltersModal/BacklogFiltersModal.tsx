...
import BacklogFiltersModalContainer from './components/BacklogFiltersModalContainer';
import modalObserver, { ModalProps } from '../../../shared/utils/observers/modalObserver';

const BacklogFiltersModal = ({ isOpen }: ModalProps) => {
    ...
    if (isShowUnsavedDataModal && !isSubmitted) {
      modalObserver.addModal(ModalNamesEnum.baseConfirmModal, {
        props: {
          ...
          handleConfirm: () => modalObserver.removeModal(ModalNamesEnum.backlogFiltersModal),
        },
      });
    } else {
      modalObserver.removeModal(ModalNamesEnum.backlogFiltersModal);
    }
  };

  return (
    <MuiDrawer isSmall isShow={!!isOpen} onClose={() => onClose()}>
      <BacklogFiltersModalContainer setIsShowUnsavedDataModal={setIsShowUnsavedDataModal} onClose={onClose} />
    </MuiDrawer>
  );
};

export default memo(BacklogFiltersModal);
