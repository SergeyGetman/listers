...
import GeneralInformationContainer from './GeneralnformationContainer';
import modalObserver, { ModalProps } from '../../../../shared/utils/observers/modalObserver';

const GarageGeneralInformationModal = ({ isOpen, props }: ModalProps) => {
    ...
    if (skipConfirmModal === false && isShowUnsavedDataModal) {
      modalObserver.addModal(ModalNamesEnum.baseConfirmModal, {
        props: {
          ...
          handleConfirm: () => {
            modalObserver.removeModal(ModalNamesEnum.garageGeneralInfo);
          },
        },
      });
    } else {
...
export default memo(GarageGeneralInformationModal);
