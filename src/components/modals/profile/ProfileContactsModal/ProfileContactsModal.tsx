...
import ProfileContactsContainer from './components/ProfileContactsContainer';
import modalObserver, { ModalProps } from '../../../../shared/utils/observers/modalObserver';

const ProfileContactsModal = ({ isOpen }: ModalProps) => {
    ...
    if (isShowUnsavedDataModal && !isSubmitted) {
      modalObserver.addModal(ModalNamesEnum.baseConfirmModal, {
        props: {
          ...
          handleConfirm: () => modalObserver.removeModal(ModalNamesEnum.profileContacts),
        },
      });
    } else {
      modalObserver.removeModal(ModalNamesEnum.profileContacts);
    }
  };

  return (
    <MuiDrawer isShow={!!isOpen} onClose={onClose}>
      <ProfileContactsContainer setIsShowUnsavedDataModal={setIsShowUnsavedDataModal} onClose={onClose} />
    </MuiDrawer>
  );
};

export default memo(ProfileContactsModal);
